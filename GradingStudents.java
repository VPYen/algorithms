...
For example, grade = 84 will be rounded to, but grade = 29
will not be rounded because the rounding would result
...
class GradingStudents {
  ...
  static int[] gradingStudents(int[] grades) {
    for(int i = 0; i < grades.length; i++) {
        int roundedGrade = grades[i] + (5 - (grades[i] % 5));
        System.out.println(grades[i] + " " + roundedGrade);
