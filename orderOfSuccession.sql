...
While the nation is mourning, the noblemen need to decide who will take the throne next.
...
The resulting table should contain the names of the King's heirs in order of their succession
to the throne as determined by their age, and preceded by their potential future titles (i.e. "King name" or "Queen name").
...
BEGIN
	SELECT
    CASE WHEN  gender = "F" THEN CONCAT("Queen ", name)
         WHEN gender = "M" THEN CONCAT("King ", name) END AS name
    FROM Successors
